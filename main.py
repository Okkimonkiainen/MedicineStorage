...
app = Flask(__name__,
            template_folder='/home/laaketilat/templates/',
            static_folder='/home/laaketilat/static/')
...
# set the secret key.  keep this really secret: Poistettu
app.secret_key = ""
...
def virheSposti(mittaus):
    with app.app_context():
        ...
        viesti=Message(subject="LÄÄKEHUONE: HÄLYTYS",
                       ...
                       recipients=[emailconfig['VASTAANOTTAJA1']],
...
def kirjaudu():
    ...
    if len(salasana)>0 and len(kayttajatunnus)>0:
        ...
        salasana=hashlib.sha512(salasana.encode('utf-8')).hexdigest()
        print(salasana)
        if salasana==kayttajansala:
            session['kayttaja']="ok"
            return redirect(url_for('paasivu'))
        else:
            virhe=True
    ...
    try:
        ...
        #Solmu 4: Avohyllyt
            ...
            try:
                ...
                return "Solmu4, tietokantaan tallennus epäonnistui"
    except Exception as err:
